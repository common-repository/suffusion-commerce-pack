...
if (!class_exists('Suffusion_Integration_Pack')) {
	abstract class Suffusion_Integration_Pack {
		...
		function other_plugins() {
			$other_plugins = array(
				'Suffusion BuddyPress Pack' => array(
					'url' => 'http://wordpress.org/extend/plugins/suffusion-buddypress-pack',
					'text' => 'BuddyPress Pack',
					'class' => 'pack-bp',
				),
				'Suffusion Commerce Pack' => array(
					'url' => 'http://wordpress.org/extend/plugins/suffusion-commerce-pack',
					'text' => 'Commerce Pack',
					'class' => 'pack-cp',
				),
				'Suffusion bbPress Pack' => array(
					'url' => 'http://wordpress.org/extend/plugins/suffusion-bbpress-pack',
					'text' => 'bbPress Pack',
					'class' => 'pack-bbp',
				),
				'Suffusion Shortcodes' => array(
					'url' => 'http://wordpress.org/extend/plugins/suffusion-shortcodes',
					'text' => 'Shortcodes',
					'class' => 'pack-sc',
				),
			);
		...
		<fieldset>
			...
			<ul class="suf-extensions">
				<?php
				foreach ($other_plugins as $name => $plugin) {
					if ($this->page_title != $name) { ?>
						<li>
							<a href="<?php echo $plugin['url']; ?>" title="<?php echo $name;?>" class="pack <?php echo $plugin['class'];?>"><?php echo $plugin['text'];?></a>
						</li>
					<?php
					}
				}
